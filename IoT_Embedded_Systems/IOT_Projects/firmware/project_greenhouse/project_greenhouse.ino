...
void loop() {
  ...
  int pulseReading = analogRead(PULSE_SENSOR_PIN);

  if (pulseReading > pulseThreshold && !pulseDetected) {
    IBI = millis() - lastBeatTime;
    lastBeatTime = millis();
    BPM = 60000.0 / IBI;
    if (BPM > 40 && BPM < 200) {
  ...
  if (pulseReading < pulseThreshold) {
    pulseDetected = false;
  }
