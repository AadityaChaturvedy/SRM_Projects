...
class TollSystem(QWidget):
    ...
    def initUI(self):
        ...
        self.log_btn.setStyleSheet("background-color: #222; color: cyan; border-radius: 10px; padding: 6px;")
                ...
                for row in reader:
                    if row and row[0] == uid:
                        ...
                        if balance >= toll_amount:
                            ...
                            self.write_log(f"{timestamp} | UID: {uid} | Name: {name} | Toll deducted | New Balance: ₹{balance:.2f}\n")
                        else:
                            self.status_label.setText("🔴 Insufficient Balance")
                            self.status_label.setStyleSheet("color: red;")
                            self.ser.write(b'DENY\n')
                            self.write_log(f"{timestamp} | UID: {uid} | Name: {name} | DENIED - Insufficient Balance: ₹{balance:.2f}\n")
                        row[2] = str(balance)
                    rows.append(row)
    ...
    def register_new_user(self, uid):
        ...
        if dialog.exec_():
            ...
            self.write_log(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | UID: {uid} | New user registered: {name}, ₹{balance}\n")
            self.process_uid(uid)

    def open_log_file(self):
        os.system("open logs.txt" if sys.platform == "darwin" else "start logs.txt" if sys.platform == "win32" else "xdg-open logs.txt")
