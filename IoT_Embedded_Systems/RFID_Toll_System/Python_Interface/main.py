...
from PyQt5.QtWidgets import (
    ...
    QProgressBar, QTextEdit, QHBoxLayout, QLineEdit, QDialog, QFormLayout
)
...
class RegisterUserDialog(QDialog):
    def __init__(self, uid):
        ...
        self.name_input.setStyleSheet("background-color: #2e2e3e; color: white; border: 1px solid #555; padding: 5px;")
        self.name_input.setFont(QFont("Consolas", 14))

        self.balance_input = QLineEdit()
        self.balance_input.setStyleSheet("background-color: #2e2e3e; color: white; border: 1px solid #555; padding: 5px;")
        self.balance_input.setFont(QFont("Consolas", 14))

        self.register_btn = QPushButton("Register")
        self.register_btn.setFont(QFont("Segoe UI", 14))
        self.register_btn.setStyleSheet("background-color: #00CED1; color: black; border-radius: 8px; padding: 6px;")
        self.register_btn.clicked.connect(self.accept)

        layout.addRow("UID:", QLabel(uid))
    ...
    def get_data(self):
        return self.name_input.text(), float(self.balance_input.text())
    ...
    def ensure_csv_exists(self):
        if not os.path.exists("users.csv"):
            with open("users.csv", "w", newline='') as f:
                writer = csv.writer(f)
                # You can include headers if you like: UID, Name, Balance
